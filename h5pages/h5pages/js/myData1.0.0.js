...
$(function(){
    ...
    function deleteFont(obj){
     ...
     function idCardchange(){
        ...
        if ( data.strIdcard.test( idDefault ) ) {
            ...
            $("#idCard").css('color','#b6b6b6').val(idDefault)
        } else{
    ...
    //如果有真实姓名，就不需要再次输入
    function namechange(){
        var idDefault = $("#readyName").attr('placeholder');
        if ( data.strReadyNam.test( idDefault ) ) {
            if ( idDefault!='请输入姓名'  ) {
                console.log( '默认真实姓名:'+idDefault )
                $("#readyName").attr('readonly','readonly')
                $("#readyName").css('color','#b6b6b6').val(idDefault)
            } 
           
        } else{
            console.log( '默认真实姓名不存在:'+idDefault )
        }
    }
